...
/*
 * Minimal, standalone sample that downloads a single S3 object to a local file
 * using ONLY the public aws-c-s3 API (no private/ headers).
 *
 * Usage:
 *   get_object --region <region> --bucket <bucket> --key <key> --output <path>
 *
...
static void s_usage(int exit_code) {
    ...
    fprintf(out, "usage: get_object --region <region> --bucket <bucket> --key <key> --output <path>\n");
    exit(exit_code);
}
...
int main(int argc, char *argv[]) {
    ...
    while (true) {
        int option_index = 0;
        int c = aws_cli_getopt_long(argc, argv, "r:b:k:o:h", s_long_options, &option_index);
        if (c == -1) {
            break;
        }
        switch (c) {
    ...
    if (region == NULL || bucket == NULL || key == NULL || output == NULL) {
        fprintf(stderr, "error: --region, --bucket, --key and --output are all required\n");
        s_usage(1);
    }
    ...
    /* Build the HTTP request describing the GET. */
    char endpoint[512];
    ...
    /* recv_filepath lets the client stream the body straight to disk for us. */
    struct aws_s3_meta_request_options options;
    AWS_ZERO_STRUCT(options);
    options.type = AWS_S3_META_REQUEST_TYPE_GET_OBJECT;
    options.message = message;
    options.recv_filepath = aws_byte_cursor_from_c_str(output);
    ...
    if (state.error_code == AWS_OP_SUCCESS) {
        printf("Downloaded s3://%s/%s to %s (HTTP %d)\n", bucket, key, output, state.response_status);
    } else {
        fprintf(
            stderr, "Download failed: %s (HTTP %d)\n", aws_error_debug_str(state.error_code), state.response_status);
        exit_code = 1;
    }
